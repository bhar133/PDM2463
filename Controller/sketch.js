...
let score = 0;
let bugs = [];
let gotBugs = [false, false, false, false, false, false, false, false, false, false,
...
function kill(){
  if (!gameOver) {
    for (let i = 0; i < bugs.length; i++) {
        let bug = bugs[i];
      if (
        circleX >= bug.sprite.x &&
        circleX <= bug.sprite.x + bug.sprite.width &&
        circleY >= bug.sprite.y &&
        circleY <= bug.sprite.y + bug.sprite.height
      ){
        gotBug = true;
      }
      if(gotBug && !gotBugs[i]){
        soundFX.player('squish').stop();
          score++;
          gotBugs[i] = true;
